    ...
    <Transition name="fade">
      <div v-if="visible" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click="close">
        <div class="bg-white rounded-3xl shadow-2xl p-8 md:p-12 max-w-3xl w-full relative" @click.stop>
          <button
            @click="close"
            class="absolute top-6 right-6 text-gray-400 hover:text-gray-800 transition-colors bg-gray-100 hover:bg-gray-200 rounded-full w-10 h-10 flex items-center justify-center">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div v-for="a in advisors" :key="a.name" class="text-center bg-slate-50 p-3 rounded-2xl border border-slate-100 hover:shadow-md transition-shadow">
              ...
              <div v-else class="w-48 h-48 mx-auto bg-white p-2 rounded-xl shadow-sm flex items-center justify-center text-gray-400 text-sm">暂无二维码</div>
            </div>
          </div>
        </div>
      </div>
